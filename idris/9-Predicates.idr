...
removeElem : (value : a) ->
             ...
             {auto prf : Elem value xs} ->
             Vect n a
removeElemWithProof value (value :: ys) { prf = Here  } = ys
removeElemWithProof { n = Z } value (y :: []) { prf = There later } = absurd later
removeElemWithProof { n = (S k) } value (y :: ys) { prf = There later } =
  y :: removeElemWithProof value ys later
